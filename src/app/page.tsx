  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="border-b border-border bg-surface">
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
          ...
          {!loading && isAuthenticated ? (
            <Link
              href="/dashboard"
              className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-primary-hover sm:w-auto sm:py-2"
            >
              {user?.avatar && (
            ...
            >
              ...
              <span className="whitespace-nowrap">Ingresar con Google</span>
            </a>
          )}
        </div>
      </header>
      ...
      <footer className="border-t border-border px-4 py-6 text-center text-sm text-text-muted">
        DevJobTracker &mdash; Hecho con Next.js + Tailwind desde Argentina
      </footer>
    </div>
  );
}
